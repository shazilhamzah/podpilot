...
def _report_summary(r):
    ...
    if not os.getenv('GROQ_API_KEY'): return fallback
    try:
        from groq import Groq
        facts={'cost':r['cost'],'security':r['security'],'drift_changes':len(r['drift']['changes'])}
        x=Groq(api_key=os.getenv('GROQ_API_KEY')).chat.completions.create(model=os.getenv('MODEL'),temperature=.2,max_tokens=140,messages=[{'role':'system','content':'Write one concise executive summary paragraph for a Kubernetes impact report.'},{'role':'user','content':'Use only these facts; mention cost/waste, security score, and reliability: '+str(facts)}])
