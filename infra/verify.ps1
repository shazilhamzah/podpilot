...
function Write-Pass    { param($Msg) Write-Host "  ✔  $Msg" -ForegroundColor Green;  $script:PassCount++ }
...
if (Get-Command az -ErrorAction SilentlyContinue)      { Write-Pass "Azure CLI is installed" }
else                                                    { Write-Host "[ERROR] Azure CLI not found — cannot continue." -ForegroundColor Red; exit 1 }

if (Get-Command kubectl -ErrorAction SilentlyContinue) { Write-Pass "kubectl is installed" }
else                                                    { Write-Host "[ERROR] kubectl not found — cannot continue." -ForegroundColor Red; exit 1 }

$null = az account show 2>&1
if ($LASTEXITCODE -eq 0) { Write-Pass "Azure CLI is authenticated" }
else                      { Write-Host "[ERROR] Not logged in — run: az login" -ForegroundColor Red; exit 1 }
...
$aksJson = az aks show --name $ClusterName --resource-group $ResourceGroup 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Fail "AKS cluster '$ClusterName' not found"
    Write-Fail "OIDC issuer NOT verifiable — cluster missing"
    Write-Fail "Workload Identity NOT verifiable — cluster missing"
    Write-Fail "Cost Analysis NOT verifiable — cluster missing"
} else {
    $aks = $aksJson | ConvertFrom-Json

    $oidcEnabled = $aks.oidcIssuerProfile.enabled
    $wiEnabled   = $aks.securityProfile.workloadIdentity.enabled
    $caProfile   = $aks.addonProfiles.PSObject.Properties["costAnalysis"]
    $caEnabled   = $caProfile -and $caProfile.Value.enabled

    if ($oidcEnabled -eq $true) { Write-Pass "OIDC issuer enabled" }
    else                        { Write-Fail "OIDC issuer NOT enabled (re-run with --enable-oidc-issuer)" }

    if ($wiEnabled -eq $true)   { Write-Pass "Workload Identity enabled" }
    else                        { Write-Fail "Workload Identity NOT enabled (re-run with --enable-workload-identity)" }

    if ($caEnabled -eq $true)   { Write-Pass "Cost Analysis add-on enabled" }
    else                        { Write-Fail "Cost Analysis add-on NOT enabled (re-run with --enable-cost-analysis)" }
}
...
try {
    ...
    Write-Fail "OpenCost allocation endpoint unreachable: $_"
} finally {
...
$cosmosJson = az cosmosdb show --name $CosmosAccount --resource-group $ResourceGroup 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Fail "Cosmos DB account '$CosmosAccount' not found"
    Write-Fail "Cosmos DB version NOT verifiable — account missing"
} else {
    $cosmos    = $cosmosJson | ConvertFrom-Json
    $cosmosKind = $cosmos.kind
    $cosmosVer  = $cosmos.apiProperties.serverVersion

    if ($cosmosKind -eq "MongoDB") { Write-Pass "Cosmos DB kind is MongoDB" }
    else                           { Write-Fail "Cosmos DB kind is '$cosmosKind' (expected MongoDB)" }

    if ($cosmosVer -eq "7.0")      { Write-Pass "Cosmos DB server version is 7.0" }
    else                           { Write-Fail "Cosmos DB server version is '$cosmosVer' (expected 7.0)" }
}
...
$uriFromOutputs = $env:MONGO_DB_URI

if (-not [string]::IsNullOrWhiteSpace($uriFromOutputs)) {
    ...
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($liveUri)) {
        Write-Pass "Connection string retrievable from Azure (not yet saved to .infra-outputs.ps1)"
    } else {
